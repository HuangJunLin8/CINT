...
\author{
  ...
  Deping Li\inst{1}\thanks{Corresponding author}  % 标记通讯作者
}
...
\institute{
  Jinan University, \\
  Zhuhai, China \\
  \email{lideping@jnu.edu.cn}  % 优先展示通讯作者邮箱 \\  % 如果会议允许只放一个邮箱
...
This task optimized the positioning of the wrist-mounted vision system. As shown in \cref{fig12}, the initial configuration (Position 1) exhibited tier recognition errors in multi-layered disc scenarios, primarily manifesting as erroneous grasps of second-layer discs with only a 40\% success rate. By relocating the camera to the robotic gripper's center position (Position 2), precise tier recognition was achieved, significantly improving the grasping success rate to 90\%.



To train a high-precision ACT (Action Chunking Transformer) model, this study implemented a systematic data collection protocol. Referencing hierarchical task decomposition methods~\cite{ref7}, for each atomic action (totaling 15 fundamental action units) in the Tower of Hanoi task, 40 training episodes were captured. Using illumination perturbation method~\cite{ref8}, with fixed lighting and background (gray tablecloth), following the robustness design principles for home environments ~\cite{ref9}. The first 20 episodes were conducted under standard lighting conditions, while the latter 20 episodes introduced lighting variations by adjusting the position and on/off state of a desk lamp to enhance the model's environmental robustness.Under our hardware configuration, the model achieved an average training time of 2 hours.


As shown in \cref{fig14}, each model achieved a final training loss below 0.05 after completing 80,000 training steps.
